...
namespace GameToolkit.Runtime.Graphics.RenderFeatures.Fog
{
    ...
    public sealed class FullScreenFogRendererFeature : ScriptableRendererFeature
    {
        ...
        class FullScreenFogRenderPass : ScriptableRenderPass
        {
            ...
            )[] ModeShaderKeywords = Enum.GetValues(typeof(FullScreenFogDensityMode))
                .Cast<FullScreenFogDensityMode>()
                .Select(mode =>
                    (
                        $"_{nameof(FullScreenFog.densityMode).ToUpper()}_{mode.ToString().ToUpper()}",
                        mode
                    )
                )
                .ToArray();
            ...
            private static readonly (
                string Name,
                FullScreenFogMode Value
            )[] DistanceModeShaderKeywords = Enum.GetValues(typeof(FullScreenFogMode))
                .Cast<FullScreenFogMode>()
                .Select(mode =>
                    ($"_{nameof(FullScreenFog.mode).ToUpper()}_{mode.ToString().ToUpper()}", mode)
                )
                .ToArray();
            ...
            )[] NoiseModeShaderKeywords = Enum.GetValues(typeof(FullScreenFogNoiseMode))
                .Cast<FullScreenFogNoiseMode>()
                .Select(mode =>
                    (
                        $"_{nameof(FullScreenFog.noiseMode).ToUpper()}_{mode.ToString().ToUpper()}",
                        mode
                    )
                )
                .ToArray();
...
#pragma warning restore CS0618
            ...
            private static void ExecuteCopyColorPass(
            ...
            )
            {
                ...
                if (FullScreenFog.UseNoiseTex(noiseMode))
                {
                    material.SetTexture(
                        NoiseTexShaderID,
                        fog.noiseTexture.value == null
                            ? Texture2D.whiteTexture
                            : fog.noiseTexture.value
                    );
                }
                if (FullScreenFog.UseNoiseIntensity(noiseMode))
                {
                    material.SetVector(
                        NoiseParamsShaderID,
                        new Vector4(
                            fog.noiseIntensity.value,
                            fog.noiseScale.value,
                            fog.noiseScrollSpeed.value.x,
                            fog.noiseScrollSpeed.value.y
                        )
                    );
                }
            ...
            )
            {
                ...
                {
                    RasterCommandBuffer rasterCmd = CommandBufferHelpers.GetRasterCommandBuffer(
                        cmd
                    );
            ...
            )
            {
                ...
                )
                {
                    ...
                    builder.SetRenderFunc(
                        (CopyPassData data, RasterGraphContext rgContext) =>
                        {
                ...
                )
                {
                    ...
                    if (
                        renderPassEvent
                        >= (RenderPassEvent)InjectionPoint.BeforeRenderingTransparents
                    )
                    {
        ...
        )
        {
            ...
            if (
                sceneView != null
                && renderingData.cameraData.camera == sceneView.camera
            ...
            _renderPass?.Setup(new PassData { Material = _material, Fog = fog, });
            //TODO: maybe we do not need color input
            _renderPass.ConfigureInput(
