...
namespace GameToolkit.Runtime.Graphics.RenderFeatures.Blur
{
    ...
    {
        public static void KawaseExecutePass<TPass, T>(TPass data, T cmd)
            where TPass : IPassData
            where T : IWrappedCommandBuffer
        {
            ...
            BlitTexture(
                ...
                blurConfig,
                mpb,
                CalculateOffset(blurConfig, 0),
                0
            );

            for (var i = 1; i < blurConfig.Iterations; i++)
            {
        ...
        static float CalculateOffset(BlurConfig blurConfig, int iteration) =>
            (blurConfig.Offset + iteration * blurConfig.Scale) / blurConfig.Downsample;

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static void BlitTexture<T>(
            T cmd,
            Texture sourceHandle,
            Texture destinationHandle,
            ...
            where T : IWrappedCommandBuffer
        {
            ...
            mpb.SetVector(
                Constants.BlurParamsId,
                new Vector4(
                    blurConfig.Intensity,
                    blurConfig.Scale,
                    blurConfig.Downsample,
                    blurConfig.Offset
                )
            );
            ...
            mpb.SetFloat(
                Constants.BlitMipLevelId,
                blurConfig.EnableMipMaps ? Mathf.Log(offset, 2) : 0
            );
