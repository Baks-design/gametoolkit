...
#pragma warning disable UDR0001 // Domain Reload Analyzer
        public static int groundLayerMask = LayerMask.GetMask(
            new string[] { "Default", "Ground", "Player clip" }
        );
...
#pragma warning restore UDR0001 // Domain Reload Analyzer
        ...
        )
        {
            ...
            var stepAngle = Vector3.Angle(
                Vector3.forward,
                new Vector3(0f, verticalStep, horizontalStep)
            );
        ...
        )
        {
            ...
            for (var bumpcount = 0; bumpcount < numBumps; bumpcount++)
            {
                ...
                // Reduce amount of mflFrameTime left by total time left * fraction
                //  that we covered.
        ...
        )
        {
            ...
            if (angle > 0f) // If the plane that is blocking us has a positive z component, then assume it's a floor.
                blocked |= 0x01; //
            if (angle == 0f) // If the plane has no Z, it is vertical (wall/step)
                blocked |= 0x02; //
